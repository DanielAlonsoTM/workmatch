...
//        firebaseAuth.addAuthStateListener(authStateListener);
        ...
        firebaseAuth.signInWithCredential(credential).addOnCompleteListener(this, new OnCompleteListener<AuthResult>() {
            ...
            public void onComplete(@NonNull Task<AuthResult> task) {
                if (task.isSuccessful()) {
                    ...
                    checkUserExists(firebaseAuth.getUid(), userData);
    ...
    private void checkUserExists(final String uuid, final HashMap<String, Object> data) {
                    ...
                    public void onComplete(@NonNull Task<DocumentSnapshot> task) {
                        if (task.isSuccessful()) {
                            if (task.getResult().exists()) {
                                UtilitiesKt.logD(FIREBASEAUTH, "Document already exists");

                                SharedPreferences pref = getSharedPreferences("pref", MODE_PRIVATE);
                                SharedPreferences.Editor editor = pref.edit();
                                editor.putString("typeUser", task.getResult().getString("typeUser"));
                                editor.commit();
                                startActivity(new Intent(ChooseRegisterActivity.this, SplashActivity.class));
                            } else {
                                FirebaseUtilsKt.createUser(data, uuid);
                                startActivity(new Intent(ChooseRegisterActivity.this, SplashActivity.class));
                            }
                        }
                    }
